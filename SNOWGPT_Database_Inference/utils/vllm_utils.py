...
#
# DeepGPT: An Audio-augmented Large Language Model for Research and Education
# Copyright (C) 2024 Deep Dive Laboratory in Buenos Aires
#
    ...
    if mode == "podgpt":
        ...
        # Evaluate our DeepGPT
            ...
            completions = llm.generate(
                prompts,
                sampling_params,
                lora_request=LoRARequest("adapter", 1, podgpt_lora)  # Load the DeepGPT LoRA to the base model
            )
            for i, output in enumerate(completions):
                temp_gen = output.outputs[0].text
                responses.append(temp_gen)
